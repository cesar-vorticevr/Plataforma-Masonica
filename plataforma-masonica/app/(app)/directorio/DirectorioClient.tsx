...
import { miPerfil, guardarPerfil } from "@/lib/data/directorio";
import { enviar } from "@/lib/data/mensajes";
import { PerfilProfesional, Logia, Usuario } from "@/lib/types";
import { initials } from "@/lib/format";

// Isla cliente del directorio: recibe los datos ya obtenidos en el servidor por props y mantiene
// la interactividad (búsqueda, modal de perfil propio, contacto). Las mutaciones usan el cliente
// de navegador; tras editar, `router.refresh()` vuelve a ejecutar el Server Component.
export default function DirectorioClient({ perfiles, logias }: { perfiles: PerfilProfesional[]; logias: Logia[] }) {
  const { user } = useAuth();
  if (!user) return null;
  return <DirectorioInner user={user} perfiles={perfiles} logias={logias} />;
}

function DirectorioInner({ user, perfiles, logias }: { user: Usuario; perfiles: PerfilProfesional[]; logias: Logia[] }) {
  ...
  return (
    <div>
      ...
      {editOpen && <EditarPerfil user={user} onClose={() => setEditOpen(false)} onSaved={() => { setEditOpen(false); router.refresh(); }} />}
...
function EditarPerfil({ user, onClose, onSaved }: { user: Usuario; onClose: () => void; onSaved: () => void }) {
  ...
  useEffect(() => {
    let activo = true;
    miPerfil(createClient(), user.id).then(perfil => {
  ...
  async function guardar() {
    ...
    const { error } = await guardarPerfil(
      createClient(), { ...p, palabras_clave: kw.split(",").map(x => x.trim()).filter(Boolean) }, user.nombre, user.logia_id);
    setGuardando(false);
