...
// Isla de mensajería: recibe el buzón inicial del servidor y refresca local con el cliente de
// navegador en cada interacción (abrir conversación, enviar) para mantener la UX del chat.
export default function MensajesClient({ inicial }: { inicial: MensajeProfesional[] }) {
  const { user } = useAuth();
  if (!user) return null;
  return <MensajesInner user={user} inicial={inicial} />;
}

interface Contacto { id: string; nombre: string }

function MensajesInner({ user, inicial }: { user: Usuario; inicial: MensajeProfesional[] }) {
  const [msgs, setMsgs] = useState<MensajeProfesional[]>(inicial);
  const [activo, setActivo] = useState<string | null>(null);
  const [txt, setTxt] = useState("");

  async function refrescar() {
    const m = await listMensajes(createClient());
    setMsgs(m);
  }
  ...
  async function abrir(cid: string) {
    setActivo(cid);
    await marcarLeidos(createClient(), cid);
  ...
  async function enviarMsg() {
    ...
    setTxt("");
    await enviar(createClient(), user.id, user.nombre, activo, activoNombre, cuerpo);
    await refrescar();
  }
